...
class IndexController extends Plugin {
		    ...
		    $reply = ' <div style="border-top: 1px solid #D7D7D7; margin-top: 10px; padding-top: 10px; margin-left: 60px;">
				...
				<div style="padding-left: 48px;">
					<div style="color: #303030; font-size: 13px; line-height: 18px; height: 26px;">
						...
						<span style="color: #aaaaaa; float: left;" class="finecms_comment_cmt_ufromname">
							(' . $comment['ip'] . ')
						</span>
					</div>
					...
					<div style="padding: 12px 5px 0 0; line-height: 14px;" class="finecms_comment_cmt_exp">
	...
	public function delAction() {
		...
		if (($this->session->is_set('user_id') && $this->session->get('user_id'))
            || (isset($this->memberinfo['id']) && ($this->memberinfo['id'] == $data['userid'] && $this->memberinfo['username'] == $data['username']))) {
			if ($data['reply']) {
