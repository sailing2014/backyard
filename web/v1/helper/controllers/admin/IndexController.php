...
class IndexController extends Admin {
	...
	public function configAction() {
	    ...
	    $string = array(
			...
			'SITE_KEY_PAGE'           => lang('a-cfg-50'),
			'SITE_KEY_CACHE'          => lang('a-cfg-51'),
			'SITE_KEY_URLRULE'        => lang('a-cfg-52'),
			'SITE_KEY_LINK'           => lang('a-cfg-53'),
			'SITE_KEYWORD_NUMS'       => lang('a-cfg-54'),
			'SITE_KEYWORD_CACHE'      => lang('a-cfg-55'),
			'SITE_KEY_URL'            => lang('a-cfg-56'),
	...
	public function cacheAction() {
	    ...
	    if ($this->get('show')) {
	        ...
	        if (empty($cache)) {
				...
				if (substr_count($version, '.') == 1) $version .= '.0';
	        ...
	        if (method_exists($c, $a)) {	
				if (isset($cache[3])) {
					$app->$a(1, $cache[3]['site']);
					$msg = lang('a-ind-' . $cache[0]) . $cache[3]['text'];
				} else {
					$app->$a(1);
					$msg = lang('a-ind-' . $cache[0]);
				}
				echo '<script type="text/javascript">window.parent.addtext("<li>' . $msg . '</li>");</script>';
				$this->adminMsg('', purl('index/cache/', array('show' => 1, 'id' => $id), 1), 0);		
			}
	    } else {
