...
class LoginController extends Admin {
    ...
    public function indexAction() {
		...
		if ($this->isPostForm()) {
		    if (isset($this->site['SITE_ADMIN_CODE']) && $this->site['SITE_ADMIN_CODE'] && !$this->checkCode($this->post('code'))) $this->adminMsg(lang('code'), url('admin/login'));
			if (cookie::get('admin_login')) $this->adminMsg(lang('a-com-25'));
		    $username = $this->post('username');
		    $password = $this->post('password');
		    $result   = $this->user->check_login($username, $password);
		    
		    if ($result) {
				if ($result['site'] && $result['site'] != $this->siteid) $this->adminMsg(lang('a-sit-23'));
    ...
    public function logoutAction() {
        if ($this->session->is_set('user_id')) $this->session->delete('user_id');
