...
class ContentController extends Admin {
    ...
    public function addAction() {
        ...
        if ($this->post('submit')) {
            ...
            //处理标签
            //if($data['catid']==24){
            $this->tags->set_tags_by_content_id($data['id'],$data['tag']);
            //}
            ...
            if($modelid==13){
                ...
                $this->model('mix_type')->set_any($result,$types);
            }elseif($modelid==21){
        ...
        //标签类目选择
        if($modelid==13){
            $checkList = $this->model('tags_type')->get_all();
        }
    ...
    public function editAction() {
        ...
        if ($this->post('submit')) {
            ...
            //处理标签
            //if($data['catid']==24) {
            $this->tags->set_tags_by_content_id($data['id'], $data['tag'], 1);
            //}
            ...
            if($modelid==13){
                ...
                $this->model('mix_type')->set_any($id,$types);
            }
        ...
        //处理标签
        $tags =$this->tags->get_tags_by_content_id($id);
        $table_data['tag']= join(',',array_map(function($n){return $n['name'];},$tags));

        //类目选择类目
        if($modelid==13){
            $types_old = $this->model('mix_type')->get_all("type_id","content_id = $id");
            foreach($types_old as $k => $v){
                $checked[$v['type_id']]=1;
            }
            $checkList = $this->model('tags_type')->get_all();
            foreach($checkList['data'] as &$v){
                $v['check'] = $checked[$v['id']];
            }
            unset($v);
        }
    ...
    public function delAction($id = 0, $catid = 0, $all = 0) {
        ...
        //处理标签
//        $this->tags->del_tags_content_id($id);

        //处理类目
        $this->model('mix_type')->del_any($id);
