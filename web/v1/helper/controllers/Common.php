...
class Common extends Controller {
    ...
    public function __construct() {
        ...
        $this->session		= $this->instance('session_db');
		...
		//定义网站常量
        define('SITE_PATH', str_replace('yard/','',self::get_base_url()));
		define('SITE_URL',		self::get_server_name() . str_replace('yard/','',self::get_base_url()));
        define('CMS_NAME',		$system_cms['name']);
        define('CMS_VERSION',	$system_cms['version']);
        define('CMS_UPDATE',	$system_cms['update']);
        define('SITE_THEME',	str_replace('yard/','',self::get_theme_url()));
	...
	*/
	...
	protected function createShow($data, $page = 1) {
		...
		$prev_page	= $this->content->getOne("`catid`=$catid AND `id`<$id AND `status`<>0 ORDER BY `id` DESC", null, 'title,url,hits,lies');
		$next_page	= $this->content->getOne("`catid`=$catid AND `id`>$id AND `status`<>0", null, 'title,url,hits,lies');
