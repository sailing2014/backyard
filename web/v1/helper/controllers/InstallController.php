	...
	public function indexAction() {
		...
		switch($step) {
			...
			case '5'://安装进度
				...
				if (strlen($password) < 4) dexit('超级管理员密码最少4位');
				if (!@mysql_connect($tdb_host, $tdb_user, $tdb_pass,$tdb_name,$tdb_port)) dexit('无法连接到数据库服务器，请检查配置');
				$tdb_name or dexit('请填写数据库名');
				if (!@mysql_select_db($tdb_name)) {
					if (!@mysql_query('CREATE DATABASE $tdb_name')) dexit('指定的数据库不存在\n\n系统尝试创建失败，请通过其他方式建立数据库');
				}
			...
			case '6'://安装成功
				...
				$data    = $user->where('username=?', $this->post('username'))->select(false);
				$cache   = new cache_file();
				$cache->set('install', 1);
				if ($data) file_put_contents(APP_ROOT . './cache/install.lock', time());
				$this->view->assign(array(
				    'percent'  => '100%',
					'username' => $this->post('username'),
					'password' => $this->post('password'),
