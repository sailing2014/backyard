...
class AttachmentController extends Common {
    ...
    public function albumAction() {
        ...
        if ($this->checkFileName($dir)) $this->attMsg(lang('m-con-20'));
...
//                'url'  => url('attachment/image', array('w' => $w, 'h' => $h, 'size' => $size, 'admin' => $this->post('admin'))),
    ...
    public function alioss_fileAction() {
        ...
        if ($this->is_post()) {
            //logdebug('**********************');
            //logdebug('开始调用文件上传');
            $data = $this->upload('file', explode(',', $type), $size, null, null, $this->get('admin'), null, $this->get('ofile'), $this->get('document'));
            //dump($data);
            //logdebug(sprintf('结束调用文件上传，返回上传结果：%s',var_export($data,true)));
            if ($data['result']) {
                $row = array(
                    'error'		=> 1,
                    'filename'  	=> ''
            ...
            //logdebug('返回结果'.var_export($row,true));
...
//                $path	= 'uploadfiles/';
        ...
        $path       = "http://oss.aliyuncs.com/".$bucket."/";
        //logdebug(sprintf('bucket path:%s',$path));
        //logdebug('实例化file_upload类');
        ...
        //logdebug('设置文件$_FILES、大小限制、文件类型');
...
//        $fname = time() . '_' . rand(10000, 99999) . '.' . $tmp_type;
        //logdebug('调用核心上传方法file_upload->upload');
        $fname = $upload->upload($path,$document.$fname);
        //logdebug('调用核心上传方法结束');
        //logdebug('返回文件路径'.$path.$fname);
//		//上传成功处理图片
//        if (!$result) $this->watermark($path . $fname);
        return array('result' => false, 'path' => $path.$fname, 'file' => $file , 'ext' => $ext);
    }
