...
class ApiController extends Common {
    ...
    public function tryoutAction(){
        ...
        $result = $this->model('tryout')->add($phone);
        switch($result){
            case 2:
                $return = array('code' => 2, 'info'=>'Repeat to add!');
                break;
            case 1:
                $return = array('code' => 1, 'info'=>'Add success!');
                break;
            default:
                $return = array('code' => 0, 'info'=>'Add failure!');
                break;
        }
    ...
    public function setColorAction(){
        ...
        if(empty($color)){
            $ret['status'] = array('code'=>400,'message'=>'color is empty');
            $this->ajaxReturn($ret);
        }
        $res = $this->model('ke_color')->set_one(array('color'=>$color));
        if($res){
            $ret['status'] = array('code'=>200,'message'=>'set color success');
        }else{
            $ret['status'] = array('code'=>400,'message'=>'set color failure');
        }
        $this->ajaxReturn($ret);
    }
    /**
    ...
    public function getColorAction(){
        $res = $this->model('ke_color')->get_all();
        foreach($res as $v){
            $data[] = $v['color'];
        }
        if($data){
            $ret['status'] = array('code'=>200,'message'=>'get color success');
            $ret['data'] = $data;
        }else{
            $ret['status'] = array('code'=>400,'message'=>'get color failure');
        }
        $this->ajaxReturn($ret);
    }
