...
class FormController extends Common {
	...
	public function postAction() {
		...
		if ($joindata) { //关联内容数据
			...
			if (empty($cdata)) $this->callback(lang('for-3', array('1'=>$joindata['modelname'], '2'=>$cid)));
		}
	    if ($this->isPostForm()) {
			$data	= $this->post('data');
			//会员投稿权限验证
		    if ($this->model['setting']['form']['code'] && !$this->checkCode($this->post('code')))	$this->callback(lang('for-4'));
			if ($this->memberPost($this->model['setting']['auth']))	$this->callback(lang('m-con-12'));
			if ($this->model['setting']['form']['post'] && empty($this->memberinfo)) $this->callback(lang('for-5'));
			if ($this->model['setting']['form']['num'] && $this->check_num($joindata, $cid)) $this->callback(lang('for-6'));
			if ($this->model['setting']['form']['ip'] && $this->check_ip($joindata, $cid)) $this->callback(lang('for-7',array('1'=>$this->model['setting']['form']['ip'])));
			if (isset($cdata['userid']) && $this->model['setting']['form']['postme'] && $this->memberinfo['id'] != $cdata['userid']) $this->callback(lang('a-mod-153'));
