...
class ContentModel extends Model {
    ...
    public function set($id, $tablename, $data) {
        ...
        //关键字处理
        if ($data['keywords']) {
            $data['keywords'] = preg_replace(array('/，/','(\s+)'),array('/,/',''),$data['keywords']);
            $tags = @explode(',', $data['keywords']);
            if (count($tags)) {
                foreach ($tags as $v) {
                    if ($v) {
                        $d = $this->from('keys', 'id')->where('name=?', $v)->select(false);
                        if (empty($d)) {
                            $sql_add_1 = 'INSERT INTO `' . $this->prefix . 'keys` (`name`,`letter`) VALUES ("' . $v . '", "' . word2pinyin($v) . '")';
                            $this->query($sql_add_1);//插入关键词
                            $keys_id = $this->get_insert_id();
                        }else{
                            $keys_id = $d['id'];
                        }
                        $sql_add_2 = "insert into fn_keys_content(keys_id,content_id) VALUE ($keys_id,$id)";//插入关键词关联
	...
	private function get_content_id() {
        ...
        if ($this->count(substr(strtolower(get_class($this)), 0, -5), '', 'id='.$id)) {
