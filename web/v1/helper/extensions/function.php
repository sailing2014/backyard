...
function thumb($img, $width = null, $height = null) {
    if (empty($img) || strlen($img) == 1 ||  !is_file($img)) return image($img);
...
function form_show_url($modelid, $data) {
    if (empty($data) || empty($modelid)) return null;
	...
	if (isset($config['show']) && $config['show']) {
		...
		if ($config['tohtml'] && $config['htmldir']) $url = $config['htmldir'] . '/' . $url;
...
function getCaturl($data, $page = 0) {
	...
	if (isset($data['setting']['url']['use']) && $data['setting']['url']['use'] == 1 && $data['setting']['url']['list']) {
		...
		if ($data['setting']['url']['tohtml'] && $data['setting']['url']['htmldir']) $url = $data['setting']['url']['htmldir'] . '/' . $url;
		$url = preg_replace('#{([a-z_0-9]+)\((.*)\)}#Uie', "\\1(safe_replace('\\2'))", $url);
		return SITE_PATH . $url;
	}
...
function dr_avatar($uid, $size = 90) {
 ...
 * Tag标签URL
 */
function key_url($word) {
    global $config;
    if (empty($word)) return '';
    return $config['SITE_KEY_URL'] ? str_replace('{key}', word2pinyin($word), SITE_PATH . $config['SITE_KEY_URL']) : url('keys/list', array('kw' => word2pinyin($word)));
}
...
function logdebug($text) {
    file_put_contents ( '/home/bbc_helper/tmp/upload_log.txt', '【' . date ( 'Y-m-d H:i:s', time () ) . '】' . $text . "\n", FILE_APPEND );
}
 ...
 * @return string
 */
function getGrowup($birthday){
    $date_1 = new DateTime($birthday);
    $time =  date('Y-m-d');
    $date_2 = new DateTime($time);
    $interval = $date_2->diff($date_1);  
    return array('y'=>$interval->y,'m'=>$interval->m,'d'=>$interval->d,'days'=>$interval->days);
}

/**
 * 根据生日获取星座
 * @param string $birthday 生日 2015-07-12
 * @return string
 */
function getZodiac($birthday)
{
    preg_match("/^(\d{4})-(\d{1,2})-(\d{1,2})$/",$birthday,$birth);
    $day   = intval($birth[3]);
    $month = intval($birth[2]);
...
function getAnimal($birthday){
    ...
    */
    ...
    include_once '../extensions/Lunar.php';
    preg_match("/^(\d{4})-(\d{1,2})-(\d{1,2})$/",$birthday,$birth);
    $lunarObj = new Lunar();
    $lunar = $lunarObj->convertSolarToLunar($birth[1],$birth[2],$birth[3]);
//    var_dump($lunar);
    return $lunar[6];
}
