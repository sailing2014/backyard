...
function kindeditor_editor($name, $content = '', $field = '') {
	...
	if ($type) { //完整模式
      ...
      function test (content) {
        $('#get_color').val(content);
        $('.ke-colorpicker-cell-top').html(\"&nbsp;\");
        $('.ke-colorpicker-cell-top').css({\"background\":content});
    ...
    $str .= '<input id="get_color" type="hidden"><textarea id="fc_' . $id . '" name="data[' . $name . ']" style="width:' . $w . '%;height:' . $h . 'px;visibility:hidden;">' . $content . '</textarea>';
	if (!isset($setting['system']) && $name == 'content') { //是否显示编辑器底部截取内容,缩略图,远程下载等提示信息
		$session  = Controller::instance('session');
		if ($session->is_set('user_id') || cookie::is_set('member_id')) {
//			$str .= '<div style="padding-top:8px;"><input type="checkbox" checked="" value="1" name="data[fn_add_introduce]"> ' . lang('a-mod-115') . '<input type="text" size="3" value="200" name="data[fn_introcude_length]" class="input-text">' . lang('a-mod-116') . '<input type="checkbox" checked="" value="1" name="data[fn_auto_thumb]"> ' . lang('a-mod-117') . ' <input type="checkbox" checked="" value="1" name="data[fn_down_image]"> ' . lang('a-mod-136') . '</div>';
                        $str .= '<div style="padding-top:8px;"><input type="checkbox" value="1" name="data[fn_add_introduce]"> ' . lang('a-mod-115') . '<input type="text" size="3" value="200" name="data[fn_introcude_length]" class="input-text">' . lang('a-mod-116') . '<input type="checkbox" value="1" name="data[fn_auto_thumb]"> ' . lang('a-mod-117') . ' <input type="checkbox" value="1" name="data[fn_down_image]"> ' . lang('a-mod-136') . '</div>';		
