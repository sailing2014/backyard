...
abstract class Controller extends Fn_base {
	...
	protected function display($file) {
	    ...
	    if (!file_exists($filename)) App::display_error(lang('app-12', array('1'=>$file)));
	    //分析视图变量
	    $this->_options['viewpath'] = SITE_PATH . basename(PLUGIN_DIR) . '/' . $this->namespace . '/views/';
	...
	protected function getParam() {
		...
		foreach ($_GET as $n=>$v) {
			if (!in_array($n, array('s', 'a', 'c'))) $param[$n] = $this->get($n);
		}
		return $param;
	}
