...
abstract class Model extends Fn_base {
	...
	public function __construct() {		
		//加载数据库配置文件
		$db     = Controller::load_config('database');
		$master = $db['mysql_master'];
       	$slave  = $db['mysql_slave'];

		if (!is_array($master)) Controller::halt('Contents of the file: config.inc.php is not correct! It must be an array.');

		//$this->db 			= mysql::getInstance($params);	
		
		$w = count($master)>1 ? floor(mt_rand(0,count($master)-1)) : 0;
        $r = count($slave)>1 ? floor(mt_rand(0,count($slave)-1)) : 0;

		$this->master = mysql::getInstance($master[$w]);
		$this->db = $this->slave  = mysql::getInstance($slave[$r]);	//默认从数据库

		$this->dbname       = $master[0]['dbname'];
		$this->prefix 		= ($master[0]['prefix']) ? trim($master[0]['prefix']) : '';
	...
	protected function _where($where, $value = null, $is_where = true) {
		...
		//当$model->where('name=?', 'finecms');操作时
		if (!is_null($value)) {
	...
	public function join($table_name, $where) {
		if (!$table_name || !$where) return false;
		if (is_array($table_name)) {									
	...
	protected function _having($where, $value = null, $is_having = true) {
		...
		//当程序$model->where('name=?', 'finecms');操作时
		if (!is_null($value)) {
			$value = $this->quote_into($value);
			$where = str_replace('?', $value, $where);
		}
	...
	public function find($id, $fields = null) {
		...
		if (is_array($id)) {			
			...
			$myrow    = $this->db->get_array($sql_str . ' ORDER BY NULL');			
		} else{			
			$sql_str .= '=' . $id;
			$myrow    = $this->db->fetch_row($sql_str . ' ORDER BY NULL');
		}
		return $myrow;
	}
	...
	public function update($data, $where, $params = null) {
		if (!is_array($data) || !$data || !$where) return false;
		//获取数据表名及字段信息
		$this->get_table_name();
		$this->get_table_fields();
		$this->get_field_type();
		...
		$content_str = implode(',', $content_array);
		unset($content_array);
		if (empty($content_str)) return false;
		//组装SQL语句
		...
		unset($this->_parts['where']);
        //echo($sql_str);
		return $this->master->query($sql_str);
	}
	...
	/**
	 * 执行SQL查询语句
	 * $all_data 是否显示全部数据开关，当为true时，显示全部数据，为false时，显示一行数据，默认为true
	 * $cache    缓存时间，单位秒。
	 */
	public function execute($sql, $all_data = true, $cache = 0) {
	    if ($cache) {
			...
			if (is_file($file) && time() - filemtime($file) < $cache) {
				...
				$data = ($all_data == true) ? $this->db->get_array($sql) : $this->db->fetch_row($sql);
	...
	public function get_tables() {
	    $result = $this->db->get_array("SHOW TABLES FROM `$this->dbname`");
	    $tables = array();
		foreach ($result as $t) {
