...
class tree extends Fn_base {
    ...
    public function get_tree($data, $parent_id = 0, $select_id = null, $pre_fix = '|-', $child = false, $modelid = null, $groupid = null, $guest = false, $roleid = null) {
        ...
        foreach ($data as $key => $value) {
            if ($value['typeid'] == 3)
                continue;
            if ($child && ($value['typeid'] != 1 && $value['child'] == 0))
                continue;
     ...
     * @return string|array
     */
    public function get_model_tree($data, $parent_id = 0, $select_id = null, $pre_fix = '|-', $modelid, $_modelid = null, $_groupid = null, $_roleid = null, $can_select_all = false) {
        if (!$data || !is_array($data))
            return '';
        $string = '';
        foreach ($data as $key => $value) {
            if ($value['typeid'] == 3 || ($value['arrmodelid'] && !in_array($modelid, $value['arrmodelid'])))
                continue;
            if ($value[$this->parentid] == $parent_id) {
                $str = '<option value=\'' . $value[$this->id] . '\'';
                if (!is_null($select_id))
                ...
                $string .= $str . '>' . ($value['parentid'] == 0 ? '' : $pre_fix) . $value[$this->name] . '</option>';
                $string .= $this->get_model_tree($data, $value[$this->id], $select_id, '&nbsp;&nbsp;' . $pre_fix, $modelid, $_modelid, $_groupid, $_roleid, $can_select_all);
            }
        }
        return $string;
    }
