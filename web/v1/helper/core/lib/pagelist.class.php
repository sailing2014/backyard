...
class pagelist extends Fn_base {
	...
	public function loadconfig($config=array()) {
	    if (empty($config)) {
	        $config = array(
				...
				'note'    => '<a>{content}</a>',
				'goto'    => '<input type="text" value="{content}" data-url="{url}">',
	        );
	    }
	 ...
	 * @return string
	 */
	private function get_goto_page(){
		$string = $this->preg_c( $this->page, $this->url, $this->config['goto']);
		return $string;
	}
	...
	public function output() {
		...
		return ($this->total <= $this->num) ? '' : $this->preg_c($this->get_note() . $this->get_goto_page() . $this->get_first_page() . $this->get_list() . $this->get_last_page() , '', $this->config['div']);
	}
