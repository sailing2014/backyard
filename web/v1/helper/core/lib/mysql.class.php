...
class mysql extends Fn_base {
	...
	public function __construct($params) {
		...
		if (!$params['host'] || !$params['username'] || !$params['dbname']) 
			Controller::halt('Mysql数据库配置文件不完整'); //检测参数信息是否完整
		
		if ($params['port'] && $params['port'] != 3306) 
			$params['host'] .= ':' . $params['port']; //处理数据库端口
		
		$this->db_link = @mysql_connect($params['host'], $params['username'], $params['password']); //实例化mysql连接ID
		
		if (!$this->db_link) 
			Controller::halt('Mysql服务器(' . $params['host'] . ')连接失败 <br/>Error Message:' . (function_exists('iconv') ? iconv('GBK', 'UTF-8', mysql_error()) : mysql_error()) . '<br/>Error Code:' . mysql_errno(), 'Warning');
		
		if (!mysql_select_db($params['dbname'], $this->db_link)) 
			Controller::halt('不能连接到数据库<br/>' . (function_exists('iconv') ? iconv('GBK', 'UTF-8', mysql_error()) : mysql_error()) . ' Error Message:' . mysql_error(), 'Warning');
