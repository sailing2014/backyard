...
class View extends Fn_base {
	...
	protected function get_compile_file($file_name) {	
		return $this->compile_dir . md5($file_name) . '.cache.php';
	}
...
//	}
	 ...
	 */
	protected function load_view_file($view_file) {        
		if (!is_file($view_file)) App::display_404_error();
		$view_content = file_get_contents($view_file);
		return $this->handle_view_file($view_content);
	}
	/**
	 ...
	 * 解析标签list
	 */
	public function _listdata($param) {
...
//            var_dump($_param);
		...
		if ($where) {
			...
			if (isset($system['extend']) && substr($system['extend'], 0, 6) == 'WHERE.') {
                ...
                /*2015/10/09 yang.f 增加vaccine范围查询 start*/
                if(isset($param['_vaccinetime'])){
                    $where .= " AND `fn_content_1_vaccine`.`vaccinetime`>=".$param['_vaccinetime'];
                }                
//                var_dump($where); exit;
                /*2015/10/09 yang.f 增加vaccine范围查询 end*/
		//延展list
		$extend = '';
		...
		if (isset($system['order']) && $system['order']) {
			if (strtoupper($system['order']) == 'RAND()') {
				$order .= ' ORDER BY RAND()';
			} else if($system['order'] == 'vaccinetime'){
                                $order .= ' ORDER BY vaccinetime ASC,updatetime DESC';                             
                        }else {
				$orders = explode(',', $system['order']);                               
				foreach ($orders as $t) {
					list($_field, $_order) = explode('_', $t);
				...
				if (substr($order,-1) == ',') $order = ' ORDER BY' . substr($order, 0, -1);
			}
		}              
		//limit与分页
		$limit = '';
