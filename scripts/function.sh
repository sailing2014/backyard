...
function install_nginx_php()
{
    ...
    which php || yum -y install php php-cli php-common php-mbstring php-mysql php-pdo php-xml php-pear php-pecl-memcache php-pecl-apc php-bcmath php-gd
    which php-fpm || return 1
    ...
    sudo service httpd configtest && sudo service httpd graceful   
    [ ! -f /etc/php-fpm.d/www.conf ] ||  sudo mv /etc/php-fpm.d/www.conf /etc/php-fpm.d/www.conf.bak
...
function do_nginx() {
    ...
    local _php_fpm_local="/home/$1/conf/php_fpm.$1.conf"
    local _php_fpm_conf="/etc/php-fpm.d/php_fpm.$1.conf"
    [ ! -f /home/$1/conf/nginx.conf ] && return 0
    [ ! -f /home/$1/conf/php_fpm.conf ] && return 0
    [ -f /home/$1/conf/nginx.conf ]  &&  sudo -u $1 mv /home/$1/conf/nginx.conf $_nginx_local
    [ -f /home/$1/conf/php_fpm.conf ] &&  sudo -u $1 mv /home/$1/conf/php_fpm.conf $_php_fpm_local
    install_nginx_php $1  || return 1
    case "$2" in
        install | update )
            if [ -f $_nginx_local ]; then
                [ -f $_php_fpm_local ]  || return 1
                sudo ln -fsn $_nginx_local $_nginx_conf
                sudo ln -fsn $_php_fpm_local $_php_fpm_conf 
